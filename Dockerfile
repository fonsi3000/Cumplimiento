...
RUN service mysql start && \
   ...
   mysql -e "CREATE USER 'luiscarrascal'@'localhost' IDENTIFIED BY 'Espumas';" && \
   mysql -e "CREATE USER 'luiscarrascal'@'%' IDENTIFIED BY 'Espumas';" && \
   mysql -e "CREATE USER 'luiscarrascal'@'127.0.0.1' IDENTIFIED BY 'Espumas';" && \
   mysql -e "GRANT ALL PRIVILEGES ON *.* TO 'luiscarrascal'@'localhost' WITH GRANT OPTION;" && \
   mysql -e "GRANT ALL PRIVILEGES ON *.* TO 'luiscarrascal'@'%' WITH GRANT OPTION;" && \
   mysql -e "GRANT ALL PRIVILEGES ON *.* TO 'luiscarrascal'@'127.0.0.1' WITH GRANT OPTION;" && \
...
# Crea un script de inicio más robusto
RUN echo '#!/bin/bash\n\
...
\n\
# Configura MySQL nuevamente por seguridad\n\
mysql -e "CREATE DATABASE IF NOT EXISTS cumplimiento_db;"\n\
mysql -e "CREATE USER IF NOT EXISTS '\''luiscarrascal'\''@'\''localhost'\'' IDENTIFIED BY '\''Espumas'\'';" \n\
mysql -e "CREATE USER IF NOT EXISTS '\''luiscarrascal'\''@'\''%'\'' IDENTIFIED BY '\''Espumas'\'';" \n\
mysql -e "CREATE USER IF NOT EXISTS '\''luiscarrascal'\''@'\''127.0.0.1'\'' IDENTIFIED BY '\''Espumas'\'';" \n\
mysql -e "GRANT ALL PRIVILEGES ON *.* TO '\''luiscarrascal'\''@'\''localhost'\'' WITH GRANT OPTION;" \n\
mysql -e "GRANT ALL PRIVILEGES ON *.* TO '\''luiscarrascal'\''@'\''%'\'' WITH GRANT OPTION;" \n\
mysql -e "GRANT ALL PRIVILEGES ON *.* TO '\''luiscarrascal'\''@'\''127.0.0.1'\'' WITH GRANT OPTION;" \n\
mysql -e "FLUSH PRIVILEGES;" \n\
